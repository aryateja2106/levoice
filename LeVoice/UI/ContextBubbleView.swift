...
/// LeVoice logo view — uses the character image, falls back to emoji.
private struct PepperLogo: View {
    var size: CGFloat = 32

    var body: some View {
        if let image = NSImage(named: "levoice-character") ?? Bundle.main.image(forResource: "levoice-character") {
            Image(nsImage: image)
                .resizable()
...
/// The LeVoice Context Bubble — a branded floating panel that shows
