...
final class SettingsDictationTestController: ObservableObject {
    ...
    func stop() {
        ...
        Task { @MainActor in
            ...
            self.lastError = text == nil ? "LeVoice could not transcribe that sample." : nil
            ...
            if !hasAccessibilityPermission || !hasInputMonitoringPermission {
                SettingsCard("Permissions") {
                    VStack(alignment: .leading, spacing: 12) {
                        ...
                        Text("Both permissions are required for hotkeys and pasting to work reliably. If LeVoice doesn't appear in Input Monitoring, click + and select it from Applications.")
                            .font(.caption)
                            .foregroundStyle(.secondary)
                    }
                }
        ...
        VStack(alignment: .leading, spacing: 24) {
            SettingsCard("Cleanup") {
                VStack(alignment: .leading, spacing: 16) {
                    ...
                    Text("When enabled, LeVoice runs local cleanup with the selected cleanup model from the Models section.")
                        .font(.caption)
                        .foregroundStyle(.secondary)
                }
            }
            ...
            SettingsCard("Cleanup prompt") {
                VStack(alignment: .leading, spacing: 10) {
                    Text("LeVoice uses this prompt before adding OCR context and correction hints.")
                        .font(.callout)
                        .foregroundStyle(.secondary)
                        .fixedSize(horizontal: false, vertical: true)
            ...
            SettingsCard("Context") {
                VStack(alignment: .leading, spacing: 16) {
                    ...
                    Text("LeVoice uses high-quality OCR on the frontmost window and adds the result to the cleanup prompt. When learning is enabled, LeVoice does a high-quality OCR check about 15 seconds after paste and only keeps narrow, high-confidence corrections.")
                        .font(.caption)
                        .foregroundStyle(.secondary)
                        .fixedSize(horizontal: false, vertical: true)
                }
    ...
    private var modelsSection: some View {
        VStack(alignment: .leading, spacing: 24) {
            SettingsCard("Speech model") {
                ...
                Text("LeVoice uses this model for speech recognition everywhere in the app.")
                    .font(.caption)
                    .foregroundStyle(.secondary)
            ...
            if transcriptionLabController.filteredEntries.isEmpty {
                if transcriptionLabController.searchText.isEmpty {
                    ContentUnavailableView(
                        ...
                        description: Text("Make a few dictations in LeVoice and they will appear here.")
                    )
        ...
        VStack(alignment: .leading, spacing: 24) {
            ...
            SettingsCard("Meeting Transcription") {
                VStack(alignment: .leading, spacing: 18) {
                    ...
                    Text("When enabled, LeVoice can detect video calls and offer to transcribe them locally. Requires Screen Recording permission for system audio capture.")
                        .font(.caption)
                        .foregroundStyle(.secondary)
...
private struct ScreenRecordingRecoveryView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 8) {
            Text("LeVoice needs Screen Recording access. Grant it in System Settings, then return to LeVoice.")
                .font(.caption)
                .foregroundStyle(.red)
