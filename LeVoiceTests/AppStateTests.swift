...
@MainActor
final class LeVoiceTests: XCTestCase {
    ...
    func testAppStateRelaunchAppSurfacesRelaunchFailures() throws {
        ...
        XCTAssertEqual(relauncher.relaunchCallCount, 1)
        XCTAssertEqual(appState.errorMessage, "Failed to relaunch LeVoice: open failed")
    }

    func testSettingsWindowHostsSwiftUIViaContentViewController() throws {
        closeWindows(titled: "LeVoice Settings")
        defer { closeWindows(titled: "LeVoice Settings") }
        let defaults = try XCTUnwrap(UserDefaults(suiteName: #function))
        defaults.removePersistentDomain(forName: #function)
        let appState = AppState(
            hotkeyMonitor: FakeHotkeyMonitor(),
        ...
        let window = try XCTUnwrap(NSApp.windows.first(where: { $0.title == "LeVoice Settings" }))
        defer { window.close() }
    ...
    func testSettingsWindowControllerCloseButtonOrdersWindowOutWithoutClosing() throws {
        ...
        let window = try XCTUnwrap(
            NSApp.windows.first(where: { $0.title == "LeVoice Settings" && $0.isVisible })
        )
    ...
    func testAppStateShowSettingsReusesSingleWindow() throws {
        ...
        let windows = NSApp.windows.filter { $0.title == "LeVoice Settings" }
        defer { windows.forEach { $0.close() } }
    ...
    func testAppStateShowDebugLogHostsSwiftUIViaContentViewController() throws {
        closeWindows(titled: "LeVoice Debug Log")
        defer { closeWindows(titled: "LeVoice Debug Log") }
        let defaults = try XCTUnwrap(UserDefaults(suiteName: #function))
        defaults.removePersistentDomain(forName: #function)
        let appState = AppState(
            hotkeyMonitor: FakeHotkeyMonitor(),
        ...
        let window = try XCTUnwrap(
            NSApp.windows.first(where: { $0.title == "LeVoice Debug Log" && $0.isVisible })
        )
        defer { window.close() }
    ...
    func testAppStateLoadsPersistedCorrectionSettingsIntoStore() throws {
        ...
        seededStore.preferredTranscriptionsText = "LeVoice\nJesse"
        ...
        XCTAssertEqual(appState.correctionStore.preferredTranscriptions, ["LeVoice", "Jesse"])
        XCTAssertEqual(
    ...
    func testAppStateUsesPreferredTranscriptionsAsOCRCustomWords() throws {
        ...
        appState.correctionStore.preferredTranscriptionsText = "LeVoice\nJesse"

        XCTAssertEqual(appState.ocrCustomWords, ["LeVoice", "Jesse"])
    }
    ...
    func testAppStateArchivesRecordingWithDiarizationSummary() async throws {
        ...
        await appState.archiveRecordingForLab(
            audioBuffer: [0.1, 0.2, 0.3],
            windowContext: OCRContext(windowContents: "LeVoice"),
